# Prompt: 修复测试缺陷 + 调度 visual-verifier 诊断（v3 最终版）

将下面 `---` 之间的完整内容复制到新 Claude Code session。串行执行。

---

> **任务**：修复 @reports/miniapp-fix-test-report.md 暴露的 5 个问题。v3 最终版——v2 审阅仅发现 Gate Check 范围过大（应只跑 Profile 测试，Fix 2 未执行时 Chat 测试必然失败），其余设计正确。
>
> **执行顺序**：Fix 1 → Fix 3 → Gate Check（仅 Profile 测试）→ Fix 4（条件）→ Fix 5 → Fix 2
...
**注意**：项目中无 ForeignKey 约束（admission_data → colleges 只是逻辑引用，普通 UUID 列），删除顺序任意。

**修复**：在 `setup_db` fixture 的清理循环（L80-83）中添加两表：
...
同时**撤销**上一轮对 `test_recommendations_no_intent_boost_when_empty` 的任何修改——恢复为原始的"创建 College + AdmissionData"版本。
...
## Gate Check: Profile E2E — 判断 Fix 4 是否需要

Fix 1 + Fix 3 完成后，**只跑 Profile 相关测试**（Fix 2 尚未执行，TestChatProfileIndicator 的 uni-input 失败是预期行为，不影响 Gate Check 判断）：

```bash
docker compose exec backend python -m pytest \
  tests/e2e/test_miniapp_profile_ux.py::TestProfilePageLogout \
  tests/e2e/test_miniapp_profile_ux.py::TestEntryAndGuestFlow \
  -v --tb=long 2>&1
```
...
- 如果仍然 FAIL → **执行 Fix 4**：将选择器改为通用 button 文本遍历
...
**修复**：
- `test_profile_page_loads`: 改用 `"我的咨询档案" in page.title()` fallback
...
**调度**：`Agent({subagent_type: "visual-verifier", model: "sonnet", description: "Diagnose missing profile-indicator", prompt: "..."})`
...
var result = {
  ...
  hasProfileTextInBody: body.indexOf('已识别') !== -1,
...
输出必须明确回答:
1. .profile-indicator 元素是否存在于 DOM？是否可见（offsetParent !== null）？
2. 如果存在但不可见，是什么 CSS 导致隐藏？
3. HTML 源码中是否包含 profile-indicator / profileSummary / handleLogout？
4. 所有 class 含 "profile" 或 "logout" 的元素列表及可见性
```

### Agent 5.2: 诊断 Profile 页 .logout-btn 缺失

**调度**：`Agent({subagent_type: "visual-verifier", model: "sonnet", description: "Diagnose missing logout-btn", prompt: "..."})`

**Prompt**：
```
你是 visual-verifier。诊断 mini-app profile 页中 .logout-btn 和 .profile-actions 元素的缺失根因。
...
3. localStorage 是否有 token？（决定 v-if 走哪个分支）
4. HTML 源码中是否包含 logout-btn / profile-actions / handleLogout？
```

### Agent 5.3: 诊断推荐页 intent_majors 数据流

**调度**：`Agent({subagent_type: "visual-verifier", model: "sonnet", description: "Diagnose intent_majors data flow", prompt: "..."})`
...
3. browser_wait_for 3000
4. browser_snapshot + browser_take_screenshot
5. browser_evaluate 执行探测（fetch 用 localhost）：
...
2. 如果 API 有返回但 DOM 为空 → 前端 loadProfile() 未正确接收
...
**修复**：只替换 `.fill()` 为 `click() + keyboard.type()`，**保留** `.send-button` 点击。加 `else` 分支记录 warning（避免输入元素不可见时静默跳过）：
...
### 总结报告写入 @reports/miniapp-fix-test-report-v3.md
...
- Gate Check: 仅跑 TestProfilePageLogout + TestEntryAndGuestFlow
- Fix 4: [执行/跳过]
- Fix 5: 3 次 visual-verifier 诊断
- Fix 2: uni-input 键盘输入 + .send-button 点击 + visible guard with warning
